...
pub fn db_path(db_dir: &Path, scope: impl DbScope) -> PathBuf {
	db_dir.join(format!("0-{}", scope.scope_name())).join(DB_FILE_NAME)
}
...
pub async fn open_db<M: Migrator + 'static>(db_dir: &Path, scope: impl DbScope) -> ThreadSafeConnection {
...
mod tests {
	...
	async fn test_db_corruption(cx: &mut gpui::TestAppContext) {
		...
		let tempdir = tempfile::Builder::new().prefix("DbTests").tempdir().unwrap();
		{
			let corrupt_db = open_db::<CorruptedDB>(tempdir.path(), release_channel::ReleaseChannel::Dev).await;
